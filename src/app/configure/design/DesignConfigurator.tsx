...
const DesignConfigurator = ({configId, imageUrl, imageDimensions}: 
    ...
    const {mutate: saveConfig} = useMutation({
        ...
        onError: () => {
            toast({
                ...
                description: "There was and error on our end. Please try again.",
                variant: "destructive"
            })
        },
        onSuccess: () => {
    ...
    return (<div className="relative mt-20 grid grid-cols-1 lg:grid-cols-3 mb-20 pb-20">
        <div ref = {containerRef} className="relative h-[37.5rem] overflow-hidden col-span-2 w-full 
        max-w-4xl flex items-center justify-center rounded-lg border-2 border-dashed border-gray-300 
        p-12 text-center focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2">
            <div className="relative w-60 bg-opacity-50 pointer-events-none aspect-[896/1831]">
                ...
                <NextImage 
                    ...
                    priority
                    alt='phone image' 
                    src='/phone-template-2.png' 
                    sizes="(max-width: 768px) 80vw, (max-width: 1200px) 40vw, 25vw"
                    className="pointer-events-none z-50 select-none"/>
                </AspectRatio>
                <div className="absolute z-40 inset-0 left-[3px] top-px right-[3px] bottom-px 
                rounded-[32px] shadow-[0_0_0_99999px_rgba(229,231,235,0.6)]"/>
                    <div className={cn("absolute inset-0 top-px left-[1px] right-[1px] bottom-px rounded-[32px]",
                        {
                            'bg-zinc-900': options.color.value === 'black',
                            'bg-blue-900': options.color.value === 'blue',
                            'bg-rose-900': options.color.value === 'rose',
                            'bg-yellow-600': options.color.value === 'yellow',
                        }
                    )}
                />
            </div>
        ...
        <div className="h-[37.5rem] w-full col-span-full lg:col-span-1 flex flex-col bg-white">
            <ScrollArea className="relative flex-1 overflow-auto">
                ...
                <div className="px-8 pb-12 pt-8">
                    <h2 className="tracking-tihgt font-bold text-3xl">
                        Customize your case
                    </h2>
                        ...
                        <RadioGroup value={options.color} onChange={(value) => {
                            ...
                            <div className="mt-3 flex items-center space-x-3">
                                {COLORS.map((color) => (
                                    ...
                                    key={color.label} 
                                    value={color}
                                    className={({active, checked}) => cn("relative -m-0.5 flex cursor-pointer items-center",
                                        "justify-center rounded-full p-0.5 active:ring-0 focus:ring-0 active:outline-none", 
                                        "focus:outline-none border-2 border-transparent", {
                                            [`border-${color.tw}`] : active || checked,
                                        })}
                                    >
                                        <span className={cn(`bg-${color.tw}`, "h-8 w-8 rounded-full border border-blac border-opacity-10")}/>
                                    </RadioGroup.Option>
                                ))}
                            </div>
                        </RadioGroup>
